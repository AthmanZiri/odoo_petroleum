...
class DateRange(models.Model):
    ...
    _order = "type_id, date_start"  # Use type_id instead of type_name for sorting
    ...
    type_id = fields.Many2one(
        ...
        string="Type",
        index=True,  # Ensure this is indexed for performance
        required=True,
        ondelete="restrict",
        domain="['|', ('company_id', '=', company_id), ('company_id', '=', False)]",
        check_company=True,
    )
    type_name = fields.Char(related="type_id.name", store=True, index=True, string="Type Name")
    ...
    def _validate_range(self):
        for this in self:
            ...
            SQL = """
                SELECT id
                FROM date_range dt
                WHERE DATERANGE(dt.date_start, dt.date_end, '[]') &&
