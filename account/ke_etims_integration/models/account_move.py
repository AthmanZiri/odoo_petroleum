...
class AccountMove(models.Model):
    ...
    def _update_receipt_content(self, content, confirmation_datetime, invoice_date, partner):
        receipt_part = {
            'custTin': (partner.vat or '')[:11],
            'rcptPbctDt': confirmation_datetime,
            'prchrAcptcYn': 'N',
        }
        if partner.contact_address_inline:
            receipt_part.update({
                'adrs': (partner.contact_address_inline or '')[:200],
            })
        content.update({
            'custTin': (partner.vat or '')[:11],
    ...
    def _get_taxes_data(self, line_items):
        tax_codes = {item['code']: item['tax_rate'] for item in self.env['ke_etims_integration.code'].search([('code_type', '=', '04')])}
    ...
    def _l10n_ke_oscu_json_from_move(self):
        ...
        content = {
            'invcNo':           '',
            'trdInvcNo':        (self.name or '')[:50],
            'orgInvcNo':        original_invoice_number,
            'cfmDt':            confirmation_datetime,
            'pmtTyCd':          self.l10n_ke_payment_method_id.code or '',
            'rcptTyCd': {
                'out_invoice':  'S',
                'out_refund':   'R',
                'in_invoice':   'P',
        ...
        if self.is_purchase_document(include_receipts=True):
            content.update({
                'spplrTin':     (self.partner_id.vat or '')[:11],
                'spplrNm':      (self.partner_id.name or '')[:60],
                'regTyCd':      'M',
                'pchsTyCd':     'N',
                'pchsSttsCd':   '02',
                'pchsDt':       invoice_date,
    ...
    def _l10n_ke_oscu_get_json_from_lines(self, tax_details):
        ...
        for index, line in enumerate(self.invoice_line_ids.filtered(lambda l: l.display_type not in ('line_section', 'line_subsection', 'line_note'))):
            ...
            line_values = {
                ...
                'itemClsCd': product.unspsc_code_id.code,
                'itemNm':    line.name,
                'pkgUnitCd': product.l10n_ke_packaging_unit_id.code,
                'pkg':       product_uom_qty / product.l10n_ke_packaging_quantity,
    ...
    def _l10n_ke_get_invoice_sequence(self):
        """ Returns the KRA invoice sequence for this invoice, creating it if needed. """
        self.ensure_one()

        sequence_code = 'ke.etims.sale.sequence' if self.is_sale_document(include_receipts=True) else 'ke.etims.purchase.sequence'
    ...
    def _l10n_ke_oscu_send_customer_invoice(self):
        ...
        if self.l10n_ke_oscu_invoice_number:
            ...
            if not error:
                data = data['receipt']
                date_str = data['sdcDateTime'].split('.')[0]
    ...
    def _get_edi_decoder(self, file_data, new=False):
        # EXTENDS 'account'
        if file_data['import_file_type'] == 'ke.etims':
            return {
                'priority': 20,
    ...
    def _get_name_invoice_report(self):
        ...
        self.ensure_one()
        if self.l10n_ke_oscu_invoice_number:
            return 'ke_etims_integration.ke_etims_report_invoice_document'
