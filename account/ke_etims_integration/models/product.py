...
from odoo.addons.ke_etims_integration.models.account_move import format_etims_datetime
...
class ProductUnspscCode(models.Model):
    ...
    Used by Kenya eTIMS integration.
    """
    ...
    l10n_ke_tax_type_id = fields.Many2one('ke_etims_integration.code')
    ...
    def _cron_ke_etims_get_codes_from_device(self):
        ...
        tax_codes = {
            ...
            for tax_code in self.env['ke_etims_integration.code'].search_read([('code_type', '=', '04')], ['code'])
        }
        last_request_date = self.env['ir.config_parameter'].get_param('ke_etims.last_unspsc_code_request_date', '20180101000000')
        ...
        self.env['ir.config_parameter'].sudo().set_param('ke_etims.last_unspsc_code_request_date', format_etims_datetime(fields.Datetime.now()))
...
class ProductTemplate(models.Model):
    ...
    unspsc_code_id = fields.Many2one(
        'product.unspsc.code', 'UNSPSC Category',
        domain=[('applies_to', '=', 'product')],
        help='The UNSPSC code related to this product. Used for eTIMS reporting in Kenya.')
...
class ProductProduct(models.Model):
    ...
    @api.depends('type')
    def _compute_l10n_ke_packaging_unit_id(self):
        service_packaging = self.env.ref('ke_etims_integration.packaging_type_ou', raise_if_not_found=False)
    ...
    def _l10n_ke_action_open_products(self, title=None):
        ...
        res = {
            ...
            'views': [(self.env.ref('ke_etims_integration.ke_etims_kra_product_tree').id, 'list'), (False, 'form')],
    ...
    def _l10n_ke_oscu_save_item_content(self):
        ...
        content = {
            'itemCd':      code,
            'itemClsCd':   self.unspsc_code_id.code or '',
            'itemTyCd':    self.l10n_ke_product_type_code,
            'itemNm':      self.name,
            'orgnNatCd':   self.l10n_ke_origin_country_id.code,
            'pkgUnitCd':   self.l10n_ke_packaging_unit_id.code,
            'qtyUnitCd':   self.uom_id.l10n_ke_quantity_unit_id.code,
            'taxTyCd':     self._l10n_ke_get_tax_type().code,
            'bcd':         self.barcode or None,
            'dftPrc':      json_float_round(self.standard_price, 2),
            'isrcAplcbYn': 'Y' if self.l10n_ke_is_insurance_applicable else 'N',
            'useYn': 'Y',
