...
# Community adaptation: these constants live in account_accountant on Enterprise.
# Deferred reports themselves need deferred_* fields that Community AML does not
# have — handlers stay importable, then raise a clear UserError at runtime.
DEFERRED_DATE_MIN = '1900-01-01'
DEFERRED_DATE_MAX = '9999-12-31'


def _deferred_entries_list_view(env):
    """Prefer Enterprise deferred tree; fall back to standard AML list."""
    view = env.ref('account_accountant.view_deferred_entries_tree', raise_if_not_found=False)
    if not view:
        view = env.ref('account.view_move_line_tree')
    return view.id


def _require_deferred_fields(env):
    if 'deferred_start_date' not in env['account.move.line']._fields:
        raise UserError(_(
            "Deferred expense/revenue reports require the Accounting app "
            "(account_accountant) fields on journal items. "
            "They are not available on Odoo Community with base_accounting_kit."
        ))
...
class AccountDeferredReportHandler(models.AbstractModel):
    ...
    def action_audit_cell(self, options, params):
        ...
        return {
            ...
            'domain': domain,
            'views': [(_deferred_entries_list_view(self.env), 'list'), (False, 'pivot'), (False, 'graph'), (False, 'kanban')],
    ...
    def open_journal_items(self, options, params):
        ...
        return {
            ...
            'res_model': 'account.move.line',
            'domain': domain,
            'views': [(_deferred_entries_list_view(self.env), 'list')],
            'context': {
    ...
    def _dynamic_lines_generator(self, report, options, all_column_groups_expression_totals, warnings=None):
        _require_deferred_fields(self.env)
